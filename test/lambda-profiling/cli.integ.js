...
describe('Creation and deployment of a Lambda project', () => {
  ...
  describe('Creation of Lambdas', () => {
    it('should be done via the sub-command "create-lambda"', () => {
      icli.catchPrintStart(showStdout);
      return icli.parse('node script.js create-lambda config-128 -r nodejs8.10 -t 30 -m 128 --dependencies inspection --role LambdaInspection'.split(' '))
      .then(res => {
        return icli.parse('node script.js create-lambda config-512 -r nodejs8.10 -t 30 -m 512 --dependencies inspection --role LambdaInspection'.split(' '));
      })
      .then(res => {
        return icli.parse('node script.js create-lambda config-1536 -r nodejs8.10 -t 30 -m 1536 --dependencies inspection --role LambdaInspection'.split(' '));
      })
      .then(res => {
        icli.catchPrintStop();
        // Create the main file of the module
  ...
  describe('Deployment of Lambdas', () => {
    it('should be done via the sub-command "deploy-lambdas"', function() {
      icli.catchPrintStart(showStdout);
      this.timeout(30000);
      return icli.parse('node script.js deploy-lambdas config-128 config-512 config-1536 -r us-east-1 -e TEST -a v0'.split(' '))
      .then(res => {
        icli.catchPrintStop();
        assert.ok(true);
      });
    ...
    it('should be done without needing to list all lambdas', function() {
      icli.catchPrintStart(showStdout);
      this.timeout(30000);
      return icli.parse('node script.js deploy-lambdas --all -r us-east-1 -e TEST -a v0'.split(' '))
      .then(res => {
        const stdout = icli.catchPrintStop();
        assert.ok(stdout.indexOf('Deploying \x1b[36m3\x1b[0m Lambda(s):') > -1);
        assert.ok(stdout.indexOf('TEST-config-128') > -1);
        assert.ok(stdout.indexOf('TEST-config-512') > -1);
        assert.ok(stdout.indexOf('TEST-config-1536') > -1);
        assert.ok(true);
      });
    });
  });
  ...
  describe('Execution of Lambdas in AWS', () => {
    it('should be done via the sub-command "test-lambda"', () => {
      icli.catchPrintStart(showStdout);
      return icli.parse('node script.js test-lambda config-128 -r us-east-1 -e TEST -a v0'.split(' '))
      .then(res => {
        return icli.parse('node script.js test-lambda config-128 -r us-east-1 -e TEST -a ""'.split(' '));
      })
      .then(res => {
        return icli.parse('node script.js test-lambda config-512 -r us-east-1 -e TEST -a v0'.split(' '));
      })
      .then(res => {
        return icli.parse('node script.js test-lambda config-1536 -r us-east-1 -e TEST -a v0'.split(' '));
      })
      .then(res => {
        icli.catchPrintStop();
        assert.ok(true);
