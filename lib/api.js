...
Api.prototype.publish = function publish() {
  ...
  .spread((awsApi, spec) => {
    if (awsApi) {
      return updateRestApi(spec, awsApi, this.apiGateway);
    } else {
      return createRestApi(spec, this.apiGateway);
    }
  })
  .then(() => {
...
/**
 * [createRestApi description]
 * @param  {[type]} apiSpec    [description]
 * @param  {[type]} apiGateway [description]
 * @return {[type]}            [description]
 */
function createRestApi(apiSpec, apiGateway) {
  console.log('Create Rest API ' + apiSpec['x-lager'].identifier);
  return Promise.promisify(apiGateway.createRestApi.bind(apiGateway))({name: apiSpec['x-lager'].identifier})
  .then((awsApi) => {
    console.log(awsApi);
    return updateRestApi(apiSpec, awsApi, apiGateway);
  });
}

/**
 * [updateRestApi description]
 * @param  {[type]} apiSpec    [description]
 * @param  {[type]} apiGateway [description]
 * @return {[type]}            [description]
 */
function updateRestApi(apiSpec, awsApi, apiGateway) {
