...
Lager.prototype.loadApi = function(apiSpecPath, identifier) {
  ...
  .spread((apiSpecPath, identifier) => {
    ...
    let api = new Api(apiSpec, { region: this.getAWSRegion() });
...
Lager.prototype.loadEndpoints = function() {
  ...
  .then(endpoints => {
    return this.fire('afterEndpointsLoad', endpoints);
  })
  .spread(endpoints => {
...
Lager.prototype.loadIntegrations = function() {
  ...
  .spread(integrationDataInjectors => {
...
Lager.prototype.addIntegrationDataToEndpoints = function(endpoints, integrationDataInjectors) {
  return this.fire('beforeAddIntegrationDataToEndpoints', endpoints, integrationDataInjectors)
  .spread((endpoints, integrationDataInjectors) => {
    return Promise.map(integrationDataInjectors, (integrationDataInjector) => {
      return Promise.map(endpoints, (endpoint) => {
        return integrationDataInjector.applyToEnpoint(endpoint);
      });
    });
  })
  .then(() => {
    return this.fire('afterAddIntegrationDataToEndpoints', endpoints, integrationDataInjectors);
  })
  .spread((endpoints, integrationDataInjectors) => {
    return Promise.resolve(endpoints);
  });
 ...
 * @return {Promise<[Api]>}
 */
Lager.prototype.addEndpointsToApis = function(apis, endpoints) {
  return this.fire('beforeAddEndpointsToApis', apis, endpoints)
  ...
  .then(() => {
    return this.fire('afterAddEndpointsToApis', apis, endpoints);
  })
  .spread((apis, endpoints) => {
    return Promise.resolve(apis);
  });
 ...
 */
Lager.prototype.publishAllApis = function(apis) {
  return this.fire('beforePublishAllApis', apis)
  .spread((apis) => {
    return Promise.map(apis, (api) => {
      return api.publish()
      .then(() => {
        return this.fire('afterPublishAllApis', apis);
      });
    });
  })
  .spread((apis) => {
 ...
 * @return {[type]}
 */
Lager.prototype.deploy = function deploy(region, stage, environment) {
  console.log(region, stage, environment);
  ...
  .spread((integrationsDataInjectors, apis, endpoints) => {
    ...
    return Promise.all([apis, this.addIntegrationDataToEndpoints(endpoints, integrationsDataInjectors)]);
  })
  .spread((apis, endpoints) => {
    console.log('Add endpoints to APIs');
    // Once the endpoints are up-to-date with the integrations, we can add them to the APIs
    return Promise.all([this.addEndpointsToApis(apis, endpoints), endpoints]);
  })
  .spread((apis, endpoints) => {
...
Lager.prototype.getApiSpec = function getApiSpec(identifier) {
  let apiSpecPath = path.join(process.cwd(), 'apis', identifier, 'spec');
  return Promise.all([this.loadApi(apiSpecPath, identifier), this.loadEndpoints()])
  .spread((api, endpoints) => {
    console.log('Add endpoints to API');
    return Promise.all([this.addEndpointsToApis([api], endpoints), endpoints]);
  })
  .spread((apis, endpoints) => {
    return console.log(util.inspect(apis[0].genSpec('doc'), { colors: true, depth: null }));
  });
};
